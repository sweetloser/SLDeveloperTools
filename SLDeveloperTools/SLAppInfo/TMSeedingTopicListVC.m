...
@implementation TMSeedingTopicListVC
...
-(UITableView *)tableview{
    if(_tableview == nil){
        _tableview = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.bounds.size.height - 64.0 - __kTopAddHeight - __kBottomAddHeight) style:UITableViewStylePlain];
...
-(void)getData:(dispatch_group_t)group{
    ...
    WS(weakSelf);
    [[NewHttpManager sharedNetManager] AmwayPost:@"plantinggrass/api/topic/topicgoList" parameters:@{@"topic_id":_topic_id,@"render_type":@"", @"page_index":[NSString stringWithFormat:@"%ld",(long)_page+1],@"page_size":@20} success:^(id  _Nonnull responseObject) {
