...
- (void)setShareAry:(NSArray *)shareAry delegate:(id)delegate type:(NSInteger)type{
    ...
    for (NSDictionary *shareDic in shareAry) {
        ...
        UIView *view = [self ittemShareViewWithFrame:frame dic:shareDic index:i type:type];
        [self addSubview:view];
    }
}

- (UIView *)ittemShareViewWithFrame:(CGRect)frame dic:(NSDictionary *)dic index:(NSInteger)index type:(NSInteger)type{
...
- (void)buttonAction:(UIButton *)sender {
    if (_MyScrollViewDelegate && [_MyScrollViewDelegate respondsToSelector:@selector(shareScrollViewButtonAction:title:index:)]) {
        [_MyScrollViewDelegate shareScrollViewButtonAction:self title:sender.titleLabel.text index:sender.tag];
    }
}
