...
#import "BXKSongSearchResualtVC.h"
...
@interface BXKSongSearchVC ()<UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate,DSKSongSearchResualtVCDelegate>
/** 表单视图 */
...
@implementation BXKSongSearchVC
...
- (void)initNavView {
    //自定义导航栏
    UIView *navView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, __kWidth, 114+__kTopAddHeight)];
    [self.view addSubview:navView];
    //删除按钮&标题
    UIView *dtView = [[UIView alloc]initWithFrame:CGRectMake(0, 20+__kTopAddHeight, __kWidth, 50)];
    ...
    //搜索
    UIView *searchView = [[UIView alloc]initWithFrame:CGRectMake(0, 70+__kTopAddHeight, __kWidth, 44)];
    ...
    UITextField *textField = [[UITextField alloc] initWithFrame:CGRectMake(16, 5, __kWidth-32, 34)];
    ...
    textField.tintColor = TextHighlightColor;
    ...
    [UIView animateWithDuration:.5 animations:^{
        ...
        textField.frame = CGRectMake(16, textField.y, __kWidth-78, textField.height);
    }];
}
...
- (void)searchResault:(NSString *)text {
    ...
    BXKSongSearchResualtVC *kssvc = [[BXKSongSearchResualtVC alloc]init];
    kssvc.delegate = self;
    kssvc.searchText = text;
    kssvc.view.frame = self.view.frame;
    [self.view addSubview:kssvc.view];
    [self addChildViewController:kssvc];
}
