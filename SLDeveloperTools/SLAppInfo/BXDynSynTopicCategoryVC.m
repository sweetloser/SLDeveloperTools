...
@interface BXDynSynTopicCategoryVC ()<JXCategoryViewDelegate>

@property (nonatomic, strong) BXDynTopicHeaderView *topicHeaderView;

@property (nonatomic, strong) JXCategoryListContainerView *listContainerView;

@property (nonatomic, strong) BXDynTopicHomeVC *topicVC;
...
@implementation BXDynSynTopicCategoryVC
...
-(void)getData{
    [HttpMakeFriendRequest GetTopicDetailWithtopic_id:self.model.topic_id Success:^(NSDictionary * _Nonnull jsonDic, BOOL flag, NSMutableArray * _Nonnull models) {
        if (flag) {
            BXDynTopicModel *model = [BXDynTopicModel new];
            [model updateWithJsonDic:jsonDic[@"data"]];
            self.topicHeaderView.model = model;
        }
...
-(void)initJXView{
    ...
    self.categoryView.backgroundColor = sl_BGColors;
    self.categoryView.titleSelectedColor = sl_textColors;
    ...
    gradientView.gradientLayer.colors = @[(__bridge id)[UIColor sl_colorWithHex:0xFF2D52].CGColor, (__bridge id)[UIColor sl_colorWithHex:0xFF2D52].CGColor];
...
-(void)AddClick{
    if ([BXLiveUser isLogin]) {
        [DynSharePopViewManager shareWithVideoId:self.model.topic_id user_Id:@"" likeNum:@"" is_zan:@"" is_collect:@"" is_follow:@"" vc:self type:1 share_type:@"dynamic"];
    }else{
