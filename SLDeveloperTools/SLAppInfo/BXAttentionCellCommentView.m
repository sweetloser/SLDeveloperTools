...
@interface BXAttentionCellCommentView()
...
@implementation BXAttentionCellCommentView
...
- (void)setupWithLikeItemsCount:(NSInteger )likeItemsCount commentCount:(NSInteger)commentCount commentItemsArray:(NSArray *)commentItemsArray
{
    ...
    _likeLabel.text = [NSString stringWithFormat:@"%ld人赞过",likeItemsCount];
    ...
    if (self.commentItemsArray.count<=2) {
        ...
        [self.moreBtn setTitle:[NSString stringWithFormat:@"查看全部%ld条评论",commentCount] forState:BtnNormal];
