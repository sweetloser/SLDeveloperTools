...
@interface CustShareView()

@property (nonatomic,assign)NSInteger index;

@end
@implementation CustShareView
...
- (void)setShareAry:(NSArray *)shareAry delegate:(id)delegate {
    ...
    [shareScrollView setShareAry:ary1 delegate:self type:1];
    ...
    if (_firstCount < shareAry.count) {
        ...
        [shareScrollView setShareAry:ary2 delegate:self type:2];
        shareScrollView.showsHorizontalScrollIndicator = _showsHorizontalScrollIndicator;
        [_boderView addSubview:shareScrollView];
    }
}
...
- (void)shareScrollViewButtonAction:(ShareScrollView *)shareScrollView title:(NSString *)title index:(NSInteger)index{
    if (_delegate && [_delegate respondsToSelector:@selector(easyCustomShareViewButtonAction:title:index:)]) {
        [self tappedCancel];
        [_delegate easyCustomShareViewButtonAction:self title:title index:index];
    }
}

- (void)cancleButtonAction:(UIButton *)sender {
    if (_delegate && [_delegate respondsToSelector:@selector(tapcancel)]) {
...
- (void)tappedCancel {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        if (weakSelf.backView) {
            weakSelf.backView.frame = CGRectMake(0, self.frame.size.height, weakSelf.backView.frame.size.width, weakSelf.backView.frame.size.height);
        }
        
        if (weakSelf.cancleButton) {
            weakSelf.cancleButton.frame = CGRectMake(0, weakSelf.cancleButton.frame.origin.y + weakSelf.backView.frame.size.height, weakSelf.cancleButton.frame.size.width, weakSelf.cancleButton.frame.size.height);
        }
        
        if (weakSelf.footerView) {
            weakSelf.footerView.frame = CGRectMake(0, weakSelf.footerView.frame.origin.y + weakSelf.backView.frame.size.height, weakSelf.footerView.frame.size.width, weakSelf.footerView.frame.size.height);
        }
        
        if (weakSelf.boderView) {
            weakSelf.boderView.frame = CGRectMake(0, weakSelf.boderView.frame.origin.y + weakSelf.backView.frame.size.height, weakSelf.boderView.frame.size.width, weakSelf.boderView.frame.size.height);
        }
        
        if (weakSelf.headerView) {
            weakSelf.headerView.frame = CGRectMake(0, weakSelf.headerView.frame.origin.y + weakSelf.backView.frame.size.height, weakSelf.headerView.frame.size.width, weakSelf.headerView.frame.size.height);
        }
...
-(void)layoutSubviews {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        if (weakSelf.backView) {
            weakSelf.backView.frame = CGRectMake(0, self.frame.size.height - weakSelf.backView.frame.size.height, weakSelf.backView.frame.size.width, weakSelf.backView.frame.size.height);
        }
